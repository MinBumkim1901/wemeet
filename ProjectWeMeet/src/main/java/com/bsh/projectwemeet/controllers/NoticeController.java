...
import com.bsh.projectwemeet.services.NoticeService;
...
public class NoticeController {
    ...
    public ModelAndView getNotice(@RequestParam(value = "p", defaultValue = "1",required = false)int requestPage,
        ...
        int searchResultCount = this.noticeService.getCount(searchCriterion, searchQuery);

        PagingModel pagingModel = new PagingModel(
                NoticeService.PAGE_COUNT,
                this.noticeService.getCount(searchCriterion, searchQuery),
                requestPage); //객체화
        NoticeWriterArticleEntity[] notice = this.noticeService.getByPage(pagingModel,searchCriterion,searchQuery);

        UserEntity user = this.noticeService.CheckUser(session);
    ...
    @ResponseBody
    public ModelAndView postNoticeWriter(HttpSession session,HttpServletRequest request, NoticeWriterArticleEntity noticeWriterArticle) {
        boolean result = this.noticeService.putNoticeWriter(session,request, noticeWriterArticle);
        ModelAndView modelAndView = new ModelAndView("home/notice/noticeWriter");
        if (result) {
//          성공적으로 작성되었다면 noticeView로 전환
            modelAndView.setViewName("redirect:/noticeView?index=" + noticeWriterArticle.getIndex());
        } else {
            modelAndView.setViewName("home/notice/noticeWriter");
    ...
    public ModelAndView getRead(@RequestParam(value = "index") int index, HttpSession session) {
        ...
        NoticeWriterArticleEntity article = this.noticeService.readArticle(index);
...
//     네트워크 페이로드 값이 upload이기 때문에 param에 upload로 만든것이다. 왜냐 정해져있는것이기 때문이다.
    public String postUploadImage(HttpServletRequest request, @RequestParam(value = "upload") MultipartFile file) throws IOException {
        NoticeWriterImagesEntity image = this.noticeService.putImage(request, file);
...
//    <> 제네릭 표현인데 알아서 공부해야한다.
    ...
    public ResponseEntity<byte[]> getDownloadImage(@RequestParam(value = "index") int index) {
        NoticeWriterImagesEntity image = this.noticeService.getImage(index);
...
//            문자열을 미디어 타입으로 뱉어낸다.
    ...
    public String deleteNoticeView(@RequestParam(value = "index") int index, HttpSession session) {
        boolean result = this.noticeService.deleteNoticeView(index, session);
    ...
    public ModelAndView patchNotice(@RequestParam(value = "index") int index, HttpSession session) {
        NoticeWriterArticleEntity article = noticeService.getPatchIndexArticle(index, session);
        ModelAndView modelAndView = new ModelAndView("home/notice/noticePatch");
        modelAndView.addObject("article", article);
    ...
    public String PatchWrite(NoticeWriterArticleEntity noticeWriterArticle,
        ...
        PatchNoticeViewResult result = this.noticeService.UpdateArticle(noticeWriterArticle, session, request);
        JSONObject responseObject = new JSONObject();
